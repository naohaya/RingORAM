...
public class Client implements ClientInterface {
	 ...
	 * 
	 * @param newdata: when operation is write, newdata is the data that you want to
	 * write
	 */
	public byte[] oblivious_access(int blockIndex, OPERATION op, byte[] newdata) {
		...
		BlockHeader block = stash.find_by_blockIndex(blockIndex);
		BlockBody blockbody;
		if (block == null) {
		...
		if (op == OPERATION.ORAM_ACCESS_WRITE) {
			if (block == null) {// not in the stash
				...
				block = new BlockHeader(blockIndex, position_new);
				blockbody = new BlockBody(newdata);
				stash.add(block);
			} else {// find in the stash, update stash block
				blockbody.setData(newdata);
	...
	public void read_block(int pathID, int blockIndex, BucketMetadata[] meta_list) {
		...
		if (found) {// add to stash
			BlockHeader blk = new BlockHeader(blockIndex, pathID);
			BlockBody blb = new BlockBody(responseBytes);
			stash.add(blk);
		}
	}
	...
	public void write_bucket(int bucket_id) {
		...
		// System.out.println("In shuffer, client WRITEBUCKET
		// successful!"+responseBytes[0]);
		sendAndGetMessage(requestBuffer, MessageUtility.ORAM_WRITEBUCKET);
		block_list = null;
		bucket_data = null;
		offset = null;
	...
	/**
	 * 16ビット整数を4ビットずつに分けてソートし、再構成して返す。
	 */
	public static void sort4BitBlocks(){
		short input = (short) 0b1101010000111001;  // 例: 0xD439
		int[] blocks = new int[4];

		// 4ビットずつに分解（MSBから）
		for (int i = 0; i < 4; i++) {
			blocks[i] = (input >> ((3 - i) * 4)) & 0xF;
		}
		// 昇順にソート
		...
		int result = 0;
		for (int i = 0; i < 4; i++) {
			result |= (blocks[i] & 0xF) << ((3 - i) * 4);
		}
		System.out.printf("元のビット列: %16s%n", String.format("%16s", Integer.toBinaryString(input & 0xFFFF)).replace(' ', '0'));
		System.out.printf("ソート後ビット列: %16s%n", String.format("%16s", Integer.toBinaryString(result & 0xFFFF)).replace(' ', '0'));
}
}